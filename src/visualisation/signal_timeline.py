...
# Виправлений імпорт registry
try:
    from api.strategies.registry import registry
    print("✅ SignalTimeline: Registry imported from api.strategies.registry")
except ImportError as e:
    print(f"❌ SignalTimeline: Failed to import from api.strategies.registry: {e}")
    try:
        from src.api.strategies.registry import registry
        print("✅ SignalTimeline: Registry imported from src.api.strategies.registry")
    except ImportError as e2:
        print(f"❌ SignalTimeline: Failed to import from src.api.strategies.registry: {e2}")
        # Створюємо заглушку
        class DummyRegistry:
            def get_strategy(self, strategy_id, params=None):
                raise ImportError("Registry not available")
            def get_strategy_info(self, strategy_id):
                return {"name": strategy_id.replace('_', ' ').title()}
...
class SignalTimeline:
    ...
    def generate_timeline(self, strategy_id: str, asset: str, days: int) -> Dict[str, Any]:
        ...
        print(f"📈 SignalTimeline generating for {strategy_id} on {asset} ({days} days)")
        ...
        try:
            ...
            # Get strategy and generate signals - ВИПРАВЛЕННЯ: передаємо порожні параметри
            print(f"   Getting strategy {strategy_id}...")
            strategy = registry.get_strategy(strategy_id, {})  # Додано порожні параметри
            print(f"   ✅ Strategy loaded: {type(strategy)}")
            
            signals = strategy.generate_signals(data, asset)
            print(f"   Signals generated: {signals.shape if hasattr(signals, 'shape') else 'No signals'}")
            ...
            print(f"❌ Error in SignalTimeline.generate_timeline: {e}")
            import traceback
            traceback.print_exc()
            raise
    ...
    def _create_timeline_chart(self, data: pd.DataFrame, signals: pd.DataFrame, trades: List, strategy_id: str, asset: str):
        ...
        print(f"   Creating timeline chart...")
