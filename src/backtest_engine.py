...
# Додаємо шлях для коректного імпорту
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))

# Виправлений імпорт
try:
    from api.strategies.registry import registry
    print("✅ Registry imported from api.strategies.registry")
except ImportError as e:
    print(f"❌ Failed to import from api.strategies.registry: {e}")
    try:
        # Альтернативний шлях
        from src.api.strategies.registry import registry
        print("✅ Registry imported from src.api.strategies.registry")
    except ImportError as e2:
        print(f"❌ Failed to import from src.api.strategies.registry: {e2}")
        # Створюємо заглушку
        class DummyRegistry:
...
class BacktestEngine:
    def __init__(self, initial_capital: float = 10000):
        ...
        print(f"🔧 BacktestEngine initialized with capital: {initial_capital}")
    ...
    def run_backtest(self, strategy_id: str, df: pd.DataFrame, asset: str, parameters: dict = None) -> Dict:
        ...
        print(f"🔧 BacktestEngine.run_backtest called: {strategy_id}")
        
        try:
            # Діагностика перед отриманням стратегії
            print(f"   Getting strategy from registry...")
            ...
            print(f"   Signals shape: {signals.shape if hasattr(signals, 'shape') else 'No shape'}")
            ...
            print(f"❌ BacktestEngine error for {strategy_id}: {e}")
            import traceback
            traceback.print_exc()
